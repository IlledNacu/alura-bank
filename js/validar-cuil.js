export default function esUnCuil(campo){
    const cuil = campo.value.replace(/[-\/]/g, "");//esto lo que hace es que si lo que fue ingresado al campo tiene un - o una /, lo reemplaza por un espacio vacío
    //console.log(cuil);
    ...
    if(tieneNumerosRepetidos(cuil)){
        ...
        if(validarPrimerosDigitos(cuil)&&validarDigitoVerificador(cuil)){
            ...
            campo.setCustomValidity("Cuil inexistente");
        }
    }
}
...
function validarPrimerosDigitos(cuil){
    let primerosDigitos = cuil.substr(0,2);//extrae los primeros dos caracteres del campo cuil
